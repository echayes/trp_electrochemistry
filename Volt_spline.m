function [y_bg, y_corr,xGrid,yGrid]=Volt_spline(ex,ey,bounds)
...
for k = 1:nTraces
    ...
    elseif size(bounds,2)==4 & bounds(k,2)~= bounds(k,3) & bounds(k,1)~= bounds(k,2);
        a_down(k,:)=linspace(min(ex(:,k)),bounds(k,1),5);
        a_mid(k,:)=linspace(bounds(k,2),bounds(k,3),1);
